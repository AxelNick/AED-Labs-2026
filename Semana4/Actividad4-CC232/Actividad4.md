...
### Estudiante
- **Nombre completo:** Axel Alberto Reyes Baldeón
- **Código:** 20200485B

### Bloque 0 - Instalación y preparación (Opcional)
...
La diferencia radica en el orden estricto de extracción de los elementos. **LIFO** (*Last In, First Out* / Pila) establece que el último elemento ingresado será el primero en ser retirado. **FIFO** (*First In, First Out* / Cola) establece que el primer elemento ingresado será el primero en retirarse, conservando el orden cronológico de llegada.
...
Se alteraron los valores de entrada $n$ y la base de destino directamente en la función principal para verificar el comportamiento de ambas variantes.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **255** | 2 | 11111111 | 11111111 | **Sí** | Verificación del límite de un byte. Ambos manejan la secuencia sin errores. |
| **2748** | 16 | ABC | ABC | **Sí** | Se comprueba que asocia correctamente los residuos mayores a 9 con caracteres alfanuméricos. |
| **100** | 8 | 144 | 144 | **Sí** | Caso de control estándar confirmando consistencia lógica en potencias de dos. |
| **42** | 5 | 132 | 132 | **Sí** | Base arbitraria; confirma que el algoritmo depende exclusivamente del fundamento aritmético. |
| **0** | 10 | 0 | 0 | **Sí** | Verificación de caso borde. La condicional inicial en ambas funciones opera correctamente. |

**Experimento 2: Análisis estructural sobre Parentheses**

Se inyectaron cadenas personalizadas en `demo_paren_rpn.cpp` para observar el comportamiento frente a fallos lógicos o sintácticos.
...
| **Texto estándar**| `"hola + mundo"` | true | true | **Sí** | El analizador ignora correctamente caracteres ajenos a los símbolos de agrupación. |
| **Sintaxis válida**| `"a * (b + c)"` | true | true | **Sí** | Procesamiento convencional esperado. |
| **Cierre excedente**| `"(a + b) )"` | false | false | **Sí** | La versión iterativa identifica un intento de `pop` sobre una estructura vacía. |
| **Cruce de llaves** | `"([)]"` | false | false | **Sí** | Detección inmediata de asimetría entre el símbolo leído y el tope de la memoria. |
| **Anidamiento** | `"{ [ ( a ) ] }"` | true | true | **Sí** | Ejecución correcta. La versión recursiva valida segmentos de manera limpia. |
| **Alta profundidad** | `"((((a))))"` | true | true | **Sí** | La función iterativa acumula elementos; la recursiva incrementa los niveles de profundidad. |
| **Error no detectado** | `"{ ( } )"` | true | **false** | **No** | Identificación de una debilidad en `parenRecursive`: no detecta el cruce al no estar diseñada para múltiples tipos de símbolos de forma cruzada. La versión iterativa demuestra mayor robustez. |
...
Se integró un nuevo operador matemático (`%`) actualizando las jerarquías en `OperatorPriority.h` y las condicionales lógicas en `ExpressionEvaluator.h` utilizando `std::fmod`.
...
Se modificó el parámetro de entrada de la función `ods::placeQueens()` para evaluar el impacto del crecimiento del tablero en el volumen de comprobaciones.

| Dimensión (N) | Soluciones válidas | Evaluaciones (`checks`) | Tendencia Observada | Análisis del Rendimiento |
| :--- | :--- | :--- | :--- | :--- |
| **N = 4** | 2 | 84 | Control | Caso base. Bajo impacto computacional. |
| **N = 5** | 10 | 276 | Crecimiento acelerado | El incremento de una sola dimensión triplica el volumen de evaluaciones lógicas. |
| **N = 6** | 4 | 1,116 | Aumento severo | A pesar de reducirse la cantidad de soluciones viables, el esfuerzo cuadruplica el escenario anterior. |
| **N = 8** | 92 | ~15,720 | Explosión combinatoria | Exhibe claramente la complejidad temporal exponencial del Backtracking sin restricciones. |

- **¿Qué patrón observas en el crecimiento de `checks`?** Se evidencia una curva de naturaleza combinatoria/factorial. El incremento unitario de la variable `N` provoca una multiplicación masiva de las comprobaciones requeridas.
- **¿Por qué contar verificaciones no es lo mismo que contar soluciones?** Las soluciones constituyen los estados finales óptimos encontrados. Las verificaciones representan el esfuerzo iterativo global del algoritmo, incluyendo el procesamiento invertido en evaluar y descartar rutas inválidas.
- **¿Dónde aparece la "poda" en este código, aunque no haya una estructura formal con ese nombre?** El mecanismo de poda se activa en el bloque condicional `while (... && conflictsWithAny(...))`. Al identificar un conflicto temprano en una fila superior, el algoritmo aborta la evaluación de todo el subárbol combinatorio de las filas inferiores, avanzando lateralmente (`++q.y`) y omitiendo millones de operaciones innecesarias.

**Experimento 5: Comportamiento ante escenarios de Laberinto**
Se inyectaron representaciones personalizadas en el vector `ods::Maze maze({...})` para observar el comportamiento dinámico de la pila.
...
| **3** | 20 | 999 | 14 | 1 | [5, 4, 4] | Variación heurística. Alterar la semilla reconfigura los tiempos de los clientes. |
| **1** | 20 | 100 | 11 | 0 | [11] | Prueba de cuello de botella: centralización de todo el flujo en una única cola. |
| **5** | 100 | 123 | 83 | 4 | [16, 16, 16, 16, 15] | Evaluación de estrés a largo plazo. La tasa de ingresos supera la capacidad de despacho. |

- **¿Qué parámetro parece influir más en la congestión observable?** El factor predominante es la duración del trámite (`servTime`). El diseño matemático expone que la probabilidad de generación de nuevos clientes supera ampliamente la frecuencia de resolución de trámites, provocando congestión.

- **¿Qué cambia cuando repites exactamente la misma semilla?** No se registran variaciones. Se demuestra la naturaleza predecible y algorítmica de los motores aleatorios controlados bajo una semilla en el software.

- **¿Qué cambia cuando mantienes `nWin` y `servTime`, pero alteras la semilla?** Se modifican las variables estocásticas a nivel micro, pero el comportamiento de congestión macroscópica se mantiene estable, evidenciando que el cuello de botella responde a deficiencias del diseño del modelo y no al azar.

- **¿Qué evidencia usarías para defender que la cola más corta es una política razonable, aunque no necesariamente óptima?** Los reportes de ocupación residual final (ej. `[16, 16, 16, 16, 15]`) demuestran que el algoritmo previene de manera muy eficaz los picos aislados de carga, distribuyendo el peso operativo de forma equitativa. No es óptimo porque no computa el acumulado del tiempo real de trámite de cada fila, solo la cantidad absoluta de clientes.
...
| :--- | :--- | :--- |
| **Conversión de base** | Pila como inversor | Transformación simple de secuencias numéricas invirtiendo los residuos aritméticos calculados. |
| **Paréntesis** | Pila como validador | Análisis de consistencia sintáctica y control de simetría en bloques de texto. |
| **N-Reinas / Laberinto** | Pila como explorador | Búsqueda con retroceso (*backtracking*). Las Reinas calculan restricciones algebraicas abstractas, el Laberinto navega una grilla física. |
| **Simulación Bancaria** | Cola como gestor | Administración de un flujo temporal estrictamente cronológico de eventos que no pueden retroceder. |

## Autoevaluación breve

- **Qué puedo defender con seguridad:** La equivalencia matemática e informática entre la recursión implícita (pila de llamadas del sistema operativo) y el control iterativo mediante pilas explícitas en el heap, gestionando de forma segura los recursos de memoria y evitando desbordamientos lógicos.

- **Qué todavía confundo:** El seguimiento detallado de estados y tokens en las dos pilas concurrentes del evaluador de expresiones complejas cuando se introducen operadores unarios encadenados o múltiples niveles de precedencia matemática no asociativa.

- **Qué experimento me dio mejor evidencia:** El Experimento 4 (N-Reinas), ya que permite visualizar de forma cuantitativa, mediante la métrica de `checks`, el impacto exponencial de la explosión combinatoria del espacio de búsqueda y cómo opera el mecanismo de la poda condicional.

- **Qué evidencia usaría en una sustentación:** Las tablas de ocupación residual simétrica obtenidas en el Experimento 6 de la simulación bancaria para defender la equidad distributiva de la política de la cola más corta, y la discrepancia exacta del error sintáctico no capturado por la recursión en el Experimento 2.